...
export default function StoreProduct({ content, onChange, editing }: Props) {
  ...
  return (
    <div style={{ background: c.theme.bg, color: c.theme.text }} className="min-h-full">
      {/* 상단 네비 */}
      <header className="sticky top-0 z-10 backdrop-blur border-b border-black/5"
        style={{ background: c.theme.bg + "e6" }}>
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-3">
          <Editable as="span" editing={editing} className="text-lg font-bold"
            value={c.brand.name}
            onChange={(v) => set({ brand: { ...c.brand, name: v } })} />
          <a href={c.brand.ctaHref}
            className="rounded-full px-4 py-2 text-sm font-semibold text-white"
            style={{ background: c.theme.primary }}>
            <Editable as="span" editing={editing} value={c.brand.ctaText}
              onChange={(v) => set({ brand: { ...c.brand, ctaText: v } })} />
          </a>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-5xl items-center gap-10 px-5 py-16 md:grid-cols-2">
        <div>
          ...
          <a href={c.brand.ctaHref}
            ...
            style={{ background: c.theme.primary }}>
            <Editable as="span" editing={editing} value={c.hero.ctaText}
              onChange={(v) => set({ hero: { ...c.hero, ctaText: v } })} />
          </a>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {c.highlights.map((h, i) => (
            <div key={i} className="rounded-2xl border border-black/5 p-6 shadow-sm">
              <Editable as="div" editing={editing} className="text-3xl"
                value={h.icon}
                onChange={(v) => {
              ...
              <Editable as="h3" editing={editing} className="mt-3 font-bold"
                value={h.title}
                onChange={(v) => {
                  const highlights = [...c.highlights];
              ...
              <Editable as="p" multiline editing={editing} className="mt-2 text-sm opacity-75"
                value={h.desc}
                onChange={(v) => {
                  const highlights = [...c.highlights];
      ...
      <section className="mx-auto grid max-w-5xl items-center gap-10 px-5 py-16 md:grid-cols-2">
        <img src={c.detail.image} alt="" className="w-full rounded-2xl object-cover shadow-lg"
          style={{ aspectRatio: "4/3" }} />
        <div>
          <Editable as="h2" editing={editing} className="text-2xl font-extrabold"
      ...
      <section className="mx-auto max-w-3xl px-5 py-12">
        <div className="overflow-hidden rounded-2xl border border-black/5">
          {c.specs.map((s, i) => (
            <div key={i}
              className="flex justify-between border-b border-black/5 px-5 py-3 text-sm last:border-0">
              <Editable as="span" editing={editing} className="font-semibold"
                value={s.label}
                onChange={(v) => {
                  const specs = [...c.specs];
                  specs[i] = { ...s, label: v };
              ...
              <Editable as="span" editing={editing} className="opacity-75"
                value={s.value}
                onChange={(v) => {
                  const specs = [...c.specs];
                  specs[i] = { ...s, value: v };
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {c.reviews.map((r, i) => (
            <div key={i} className="rounded-2xl border border-black/5 p-6 shadow-sm">
              ...
              <Editable as="p" multiline editing={editing} className="mt-2 text-sm"
                value={r.text}
                onChange={(v) => {
              ...
              <Editable as="div" editing={editing} className="mt-3 text-xs font-semibold opacity-60"
                value={r.name}
                onChange={(v) => {
                  const reviews = [...c.reviews];
                  reviews[i] = { ...r, name: v };
      ...
      {/* 가격 / 구매 CTA */}
      <section id="pricing" className="mx-auto max-w-3xl px-5 py-16 text-center">
        <div className="rounded-3xl border border-black/5 p-10 shadow-sm">
          <div className="flex items-end justify-center gap-3">
            <Editable as="span" editing={editing} className="text-4xl font-extrabold"
              value={c.pricing.price}
              onChange={(v) => set({ pricing: { ...c.pricing, price: v } })} />
            <Editable as="span" editing={editing} className="text-lg line-through opacity-40"
              value={c.pricing.compareAt}
              onChange={(v) => set({ pricing: { ...c.pricing, compareAt: v } })} />
          </div>
          <Editable as="p" editing={editing} className="mt-2 text-sm opacity-70"
            value={c.pricing.note}
            onChange={(v) => set({ pricing: { ...c.pricing, note: v } })} />
          <a href="#"
            className="mt-6 inline-block rounded-lg px-10 py-4 text-lg font-bold text-white"
            style={{ background: c.theme.primary }}>
            <Editable as="span" editing={editing} value={c.pricing.ctaText}
              onChange={(v) => set({ pricing: { ...c.pricing, ctaText: v } })} />
          </a>
        </div>
      </section>
        ...
        <div className="space-y-4">
          {c.faq.map((f, i) => (
            <div key={i} className="rounded-xl border border-black/5 p-5">
              <Editable as="div" editing={editing} className="font-semibold"
                value={f.q}
                onChange={(v) => {
                  const faq = [...c.faq];
                  faq[i] = { ...f, q: v };
      ...
      {/* 푸터 */}
      <footer className="border-t border-black/5 px-5 py-10 text-center text-xs opacity-60">
        <Editable as="span" multiline editing={editing} value={c.footer.text}
          onChange={(v) => set({ footer: { ...c.footer, text: v } })} />
      </footer>
    </div>
  );
}
